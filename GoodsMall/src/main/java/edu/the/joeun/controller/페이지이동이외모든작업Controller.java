 ...
 *
 *프론트엔드와 백엔드는 서로 어디서 어떤 데이터를 주고 받을지 주소에 대한 약속 기약하고,
 * 프론트엔드는 ajax 나 fetch promise 내에서 백엔드로 데이터를 전달하거나 조회할 주소 작성
 * 백    엔드는 @RestController 내부에서 @GetMapping 이나 @PostMapping 등
...
public class 페이지이동이외모든작업Controller {
     ...
     * 3. Service 는 GoodsMapper 를 통해 DB 에서 모든 상품 데이터 조회
 ...
 */
     ...
     *     -> @RequestBody  을 이용해서 JSON 데이터를 Java  형태로 맞춰서 변환해주는 어노테이션
     * 3. GoodsService의 insertGoods() 메서드 호출
     * 4. Service 는 GoodsMapper 를 통해 DB에 상품 데이터 저장
     ...
     *                JavaScript에 저장된 데이터를 Goods 객체 로 전달 받은 상태
     * @return void - 반환값 없으며, 저장 성공시 (HTTP 200 OK 상태 코드만 전달)
     *
     * -> 개발자의 코드가 문제가 생겼을 경우 AI 에서는 상태 코드 확인을 추가하라 할 것
     ...
     *
     * @param user = html -> js 로 가져온 유저 정보 데이터를 모두 json 형태 설정
     ...
     * 매개변수와 파라미터는 빈값 형태
     * @return mybatis에 작성된 sql -> mapper 조회 후, service 로 전달해온 데이터를
