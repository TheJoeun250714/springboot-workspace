...
public class BoardScheduling {
    // 오버라이드 되어 사용된 기능을 사용하기 때문에 Service 나 ServiceImpl 가져와서 사용
     ...
     */
    // controller에 작성하지 않아도 알아서 11시 59분이 되면
    // 자동으로 데이터 저장과 삭제를 진행할 것이기 때문에
    // postMapping 이나 deleteMapping을 따로 api 설정해서 연결하지 않아도된다.
    // 소비자한테 보여주는 것이 아니라, 우리 회사의 페이지를 위해서
    // 개발자가 자동업데이트 처리를 진행하는 것이기 때문
    // dev -         stage       - prod
    // 개발 - 배포전 회사 테스트   실제배포
    ...
    public void updatePopularBoards(){
        ...
        try {
            // result 내부에는 delete 한 후, insert를 몇 개완성했는지
            // db에 저장되는 개수가 저장될 것
          int result =  schedulingService.updatePopularBoards();
          // result 라는 공간에 저장된 개수를 log 로 확인
          // "" 사이에 들어갈 데이터 값
